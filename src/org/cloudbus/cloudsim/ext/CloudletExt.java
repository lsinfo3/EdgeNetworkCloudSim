...
public class CloudletExt extends SimEntity {
	 ...
	 */
	private final double lifeLength = 0;
	...
	public void setUserId(final int id) {
		userId = id;
		if (record) {
			write("Assigns the Cloudlet to " + CloudSim.getEntityName(id)
					+ " (ID #" + id + ")");
		}
	}
	...
	public void setResourceParameter(final int resourceID, final double cost) {
		...
		if (index == -1 && record) {
			write("Allocates this Cloudlet to " + res.resourceName + " (ID #"
	...
	public void setCloudletStatus(final int newStatus) throws Exception {
		...
		if (newStatus < Cloudlet.CREATED
				|| newStatus > Cloudlet.FAILED_RESOURCE_UNAVAILABLE) {
			throw new Exception(
		...
		if (record) {
			write("Sets Cloudlet status from " + getCloudletStatusString()
					+ " to " + Cloudlet.getStatusString(newStatus));
		}
	...
	protected void write(final String str) {
		...
		if (num == null || history == null) { // Creates the history or
			...
			history.append("Time (sec)       Description Cloudlet #"
					+ cloudletId);
			history.append(System.getProperty("line.separator"));
			history.append("------------------------------------------");
			history.append(System.getProperty("line.separator"));
	 ...
	 * @post $none
	 */
	public void setResourceParameter(final int resourceID,
			final double costPerCPU, final double costPerBw) {
	 ...
	 */
	public void setUtilizationModelCpu(
			final UtilizationModel utilizationModelCpu) {
	 ...
	 */
	public void setUtilizationModelRam(
			final UtilizationModel utilizationModelRam) {
	...
	public void processEvent(SimEvent ev) {
		if (!started) {
			...
			// Tell the cloudlet to destroy itself after its lifeline.
			if (getLifeLength() > 0) {
				send(getId(), getLifeLength(),
						CloudSimTagsExt.CLOUDLET_DESTROY_ITSELF_NOW, null);
			}
		}
